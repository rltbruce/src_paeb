...
(function ()
{
    ...
    angular
        .module('app.paeb.gerer_subvention_financiere.niveau_ufp_feffi.convention_suivi_f_ufp')        
...
/*******************************************Debut maitrise d'oeuvre*************************************/        
        ...
        apiFactory.getAPIgeneraliserREST("convention_cisco_feffi_entete/index",'menu','getconventionNeeddemandefeffivalidationufp').then(function(result)
        {
       ...
       /* {titre:"Avancement"
        ...
        vm.step_menu_demande = function()
        {
          ...
          vm.showbuttonValidationufp = false;
          vm.showbuttonValidationenrejeufp = false;
          apiFactory.getAPIgeneraliserREST("demande_realimentation_feffi/index","menu","getdemandeemiufpByconvention",'id_convention_cife_entete',vm.selectedItemConvention_entete.id).then(function(result)
          {
...
/*****************Debut StepTwo demande_realimentation_feffi****************/
      ...
      vm.demande_realimentation_column = [                
        ...
        vm.situationde = function(validation) //ts miasa
        {
            switch (validation)
            {
              case '1':
  ...
  /*************************************debut StepThree transfert ufp********************************/
          //col table
            ...
            function ajoutTransfert_ufp(transfert_ufp,suppression)
            {
                if (NouvelItemTransfert_ufp==false)
                {
                    apiFactory.getAPIgeneraliserREST("transfert_daaf/index",'menu',"gettransfert_daafvalideById",'id_transfert_daaf',transfert_ufp.id).then(function(result)
                    {
