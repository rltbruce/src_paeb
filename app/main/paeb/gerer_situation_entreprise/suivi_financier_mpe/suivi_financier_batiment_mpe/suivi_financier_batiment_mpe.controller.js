(function ()
{
    ...
    angular
        ...
        .controller('Suivi_financier_batiment_mpeController', Suivi_financier_batiment_mpeController);
    /** @ngInject */
    function Suivi_financier_batiment_mpeController($mdDialog, $scope, apiFactory, $state,apiUrl,$http,$cookieStore)
    {
		    var vm = this;
        ...
        vm.selectedItemDemande_batiment_prestataire = {};
        vm.alldemande_batiment_prestataire = [];
...
/**********************************fin contrat prestataire****************************************/
        ...
        vm.selectionContrat_prestataire = function (item)
        {
            ...
            if (item.id!=0)
            {
              apiFactory.getAPIgeneraliserREST("demande_batiment_prestataire/index",'menu','getdemandedisponibleBycontrat','id_contrat_prestataire',item.id).then(function(result)
              {
                  vm.alldemande_batiment_prestataire = result.data.response;
                  console.log(vm.alldemande_batiment_prestataire);
              });
              vm.stepOne = true;
              vm.stepTwo = false;
              vm.stepThree = false;
...
/**********************************fin contrat prestataire****************************************/
      ...
      vm.demande_batiment_prestataire_column = [
        ...
        vm.selectionDemande_batiment_prestataire= function (item)
        {
            vm.selectedItemDemande_batiment_prestataire = item;
           if(item.id!=0)
           {
            apiFactory.getAPIgeneraliserREST("paiement_batiment_prestataire/index",'id_demande_batiment_pre',item.id).then(function(result)
            {
            ...
            console.log(vm.selectedItemDemande_batiment_prestataire) ;
        };
        $scope.$watch('vm.selectedItemDemande_batiment_prestataire', function()
        {
             if (!vm.alldemande_batiment_prestataire) return;
             vm.alldemande_batiment_prestataire.forEach(function(item)
             {
                item.$selected = false;
             });
             vm.selectedItemDemande_batiment_prestataire.$selected = true;
        });
