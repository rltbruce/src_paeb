(function ()
{
    ...
    angular
        ...
        .controller('Suivi_financier_latrine_mpeController', Suivi_financier_latrine_mpeController);
    /** @ngInject */
    function Suivi_financier_latrine_mpeController($mdDialog, $scope, apiFactory, $state,apiUrl,$http,$cookieStore)
    {
		    var vm = this;
        ...
        vm.selectedItemDemande_latrine_prestataire = {};
        vm.alldemande_latrine_prestataire = [];
...
/**********************************fin contrat prestataire****************************************/
        ...
        vm.selectionContrat_prestataire = function (item)
        {
            ...
            if (item.id!=0)
            {
              apiFactory.getAPIgeneraliserREST("demande_latrine_prestataire/index",'menu','getdemandedisponibleBycontrat','id_contrat_prestataire',item.id).then(function(result)
              {
                  vm.alldemande_latrine_prestataire = result.data.response;
                  console.log(vm.alldemande_latrine_prestataire);
              });
              vm.stepOne = true;
              vm.stepTwo = false;
              vm.stepThree = false;
...
/**********************************fin contrat prestataire****************************************/
      ...
      vm.demande_latrine_prestataire_column = [
        ...
        vm.selectionDemande_latrine_prestataire= function (item)
        {
            vm.selectedItemDemande_latrine_prestataire = item;
           if(item.id!=0)
           {
            apiFactory.getAPIgeneraliserREST("paiement_latrine_prestataire/index",'id_demande_latrine_pre',item.id).then(function(result)
            {
            ...
            console.log(vm.selectedItemDemande_latrine_prestataire) ;
        };
        $scope.$watch('vm.selectedItemDemande_latrine_prestataire', function()
        {
             if (!vm.alldemande_latrine_prestataire) return;
             vm.alldemande_latrine_prestataire.forEach(function(item)
             {
                item.$selected = false;
             });
             vm.selectedItemDemande_latrine_prestataire.$selected = true;
        });
