(function ()
{
    ...
    angular
        ...
        .controller('Suivi_financier_fin_travaux_moeController', Suivi_financier_fin_travaux_moeController);
    /** @ngInject */
    function Suivi_financier_fin_travaux_moeController($mdDialog, $scope, apiFactory, $state,apiUrl,$http,$cookieStore)
    {
       var vm = this;
...
/**********************************fin contrat prestataire****************************************/
        ...
        vm.selectionContrat_bureau_etude = function (item)
        {
            ...
            if (item.id!=0)
            {
              apiFactory.getAPIgeneraliserREST("demande_fin_travaux_moe/index",'menu','getdemandedisponibleBycontrat','id_contrat_bureau_etude',item.id).then(function(result)
              {
