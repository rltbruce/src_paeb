(function ()
{
    ...
    function Demande_latrine_moeController($mdDialog, $scope, apiFactory, $state,apiUrl,$http,$cookieStore,apiUrlFile)
    {
        ...
        vm.alldemande_latrine_moe_invalide = [];
...
//fonction selection item latrine construction cisco/feffi
        vm.selectionlatrine_construction = function (item)
        {
            ...
            {  
              ...
              apiFactory.getAPIgeneraliserREST("demande_latrine_moe/index",'menu','getalldemandeBylatrine','id_latrine_construction',item.id).then(function(result)
              {
                  vm.alldemande_latrine_moe_invalide = result.data.response.filter(function(obj)
                  {
...
/**********************************fin latrine construction****************************************/
        ...
        apiFactory.getOne("utilisateurs/index", id_user).then(function(result)             
        {
          ...
          if (usercisco.id!=undefined)
          {
            ...
            apiFactory.getAPIgeneraliserREST("demande_latrine_moe/index",'menu','getalldemandeinvalideBycisco','id_cisco',usercisco.id).then(function(result)
              {
                  vm.alldemande_latrine_moe_invalide = result.data.response;
                  console.log(vm.alldemande_latrine_moe_invalide);
              });
...
//col table
        ...
        { 
           var items = {
            ...
            {
                vm.alldemande_latrine_moe_invalide.push(items);                        
                vm.alldemande_latrine_moe_invalide.forEach(function(mem)
                {
        ...
        { 
          ...
          apiFactory.getAPIgeneraliserREST("demande_latrine_moe/index",'menu','getalldemandeBycontrat','id_contrat_bureau_etude',item.id_contrat_bureau_etude).then(function(result)
          {
              if (NouvelItemDemande_latrine_moe ==false)
              {
                vm.alldemande_latrine_moe = result.data.response.filter(function(obj){return obj.id == item.id;});
          ...
          apiFactory.getAPIgeneraliserREST("demande_debut_travaux_moe/index",'menu','getalldemandeByContrat','id_contrat_bureau_etude',item.id_contrat_bureau_etude).then(function(result)
          {
              ...
              if (vm.alldemande_debut_travaux_moe.length==vm.alltranche_d_debut_travaux_moe.length)
              {
                  ...
                  {
                      var last_id_demande = Math.max.apply(Math, vm.alldemande_latrine_moe.map(function(o){ return o.id;}));
                      vm.dernierdemande = vm.alldemande_latrine_moe.filter(function(obj){return obj.id == last_id_demande;});
                      var numcode=vm.dernierdemande[0].tranche.code.split(' ')[1];

                      if (vm.dernierdemande[0].validation==3)
                      {

                          vm.allcurenttranche_demande_latrine_moe = vm.alltranche_demande_latrine_moe.filter(function(obj){ return obj.code == 'tranche '+(parseInt(numcode)+1);});
                      } 
                      else
                      {
        ...
        { 
          ...
          /*if(vm.alldemande_latrine_moe_invalide.length>0)
          {
            var last_id_demande = Math.max.apply(Math, vm.alldemande_latrine_moe_invalide.map(function(o)
            { 
              return o.id;
            }));

            vm.dernierdemande = vm.alldemande_latrine_moe_invalide.filter(function(obj)
            {
       ...
       // };
        ...
        vm.annulerDemande_latrine_moe = function(item)
        {
          ...
          {
            vm.alldemande_latrine_moe_invalide = vm.alldemande_latrine_moe_invalide.filter(function(obj)
            {
        ...
        {
             if (!vm.alldemande_latrine_moe_invalide) return;
             vm.alldemande_latrine_moe_invalide.forEach(function(item)
             {
                item.$selected = false;
             });
        ...
        vm.modifierDemande_latrine_moe = function(item)
        {
            ...
            $scope.vm.alldemande_latrine_moe_invalide.forEach(function(mem) {
              mem.$edit = false;
            });
        ...
        {          
            if (suppression!=1)
            {
               var pass = vm.alldemande_latrine_moe_invalide.filter(function(obj)
                {
                ...
                if(pass[0])
                {
                   if((pass[0].objet   != currentItemDemande_latrine_moe.objet )
                    || (pass[0].id_contrat_bureau_etude   != currentItemDemande_latrine_moe.id_contrat_bureau_etude )
        ...
        function insert_in_baseDemande_latrine_moe(demande_latrine_moe,suppression)
        {
            ...
            {   
                ...
                {
                    return obj.id == demande_latrine_moe.id_contrat_bureau_etude;
                });
                ...
                if (NouvelItemDemande_latrine_moe == false)
                {
                    ...
                    if(suppression==0)
                    {
                        ...
                        vm.selectedItemDemande_latrine_moe.contrat_bureau_etude = contra[0] ;
                ...
                {
                  demande_latrine_moe.contrat_bureau_etude = contra[0] ;
        ...
        { 
          ...
          var reste = 0;
          var anterieur = 0;
          var montant = (parseInt(contra[0].montant_contrat) * vm.allcurenttranche_demande_latrine_moe[0].pourcentage)/100;
          var cumul = montant;
        ...
        vm.validerDemande_latrine_moe = function()
        {
        ...
        {console.log(demande_latrine_moe);
            //add
            var config =
            {
                headers : {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'}
